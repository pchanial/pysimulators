...
def test_convolution_truncated_exponential_morphing():
    def func1(tau):
        assert_is_type(ConvolutionTruncatedExponentialOperator(tau), IdentityOperator)
    ...
    def func2(tau):
        assert_is_type(
            ConvolutionTruncatedExponentialOperator(tau),
            ConvolutionTruncatedExponentialOperator,
        )
...
def test_spherical():
    ...
    def func(cls_sph, cls_car, cin, cout, v, s, d):
        if 'Horizontal' in str(cls_sph):
            ...
            args = ()
        op_sph = cls_sph(*args, conventionin=cin, conventionout=cout, degrees=d)
        actual = op_sph(v)
