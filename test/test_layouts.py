...
def test_special_attribute_array():
    ...
    for setattr_, v in zip(
        (layout.setattr_unpacked, layout.setattr_packed),
...
def test_center():
    ...
    for setattr_, v in zip(
        ('setattr_unpacked', 'setattr_packed'), (vertex, vertex.reshape((-1, 4, 2)))
    ):
        yield func, setattr_, v
...
def test_packunpack():
    ...
    index2 = index1.copy()
    index2[0, 2] = -1
    removed = [[True, False, False], [False, False, False], [True, False, True]]
    ...
    def func(r, i, d):
        layout = Layout(shape, vertex=vertex, center=center, removed=r, index=i)
        ...
        out_packed = np.empty((len(layout.packed),) + d.shape[2:], d.dtype).view(
            type(d)
        )
...
def test_layout_grid_circles_unit():
    shape = (3, 2)
    removed = [[False, True], [False, False], [False, False]]
