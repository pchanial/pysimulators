...
The @Layout@ class is very flexible and is meant to facilitate the ordering, the slicing and the MPI-distribution of a set of elements with associated properties. Such elements can be for instance map pixels, time samples with associated astrometry or detectors of given geometries and characteristics. An @Acquisition@ instance will have at least 3 @Layout@ attributes:

    * @scene@
    * @instrument.detector@
    * @sampling@

The first one holds the shape of the acquisition's input @scene.shape@ and the other two the shape of the acquisition's output @(len(instrument.detector), len(sampling)@. Here are examples of @Layout@ subclasses:
...
A @Layout@ instance stores information about its components, such as the spatial location or any property.
...
>>> gain = [[1.0, 1.2, 1.5],
            [0.9, 1.0, 1.0],
            [0.8, 1.0, 1.0]]
>>> detectors = LayoutSpatialGridSquares(shape, spacing, selection=selection,
                                         gain=gain)
>>> detectors.shape
(3, 3)
>>> len(detectors)
...
>>> ordering = np.vstack([np.hstack([i, i + 16]),
                          np.hstack([i + 32, i + 48])])
>>> detectors = LayoutSpatialGridSquares((8, 8), 0.250, ordering=ordering)
>>> detectors.plot()
...
h2. 3.1.4. Layout slicing and splitting

A @Layout@ instance can be sliced and iterated over:

{% highlight python %}
>>> samples = LayoutTemporal(100, angle=np.arange(100) * 360 / 100)
...
h2. 3.1.5. Layout MPI-scattering

During the execution of an MPI job, a @Layout@ instance can be MPI-scattered, i.e. distributed to a set of MPI processes. Accessing the unpacked attributes through the @all@ attribute or calling the method @unpack@ will MPI-gather the local arrays into a global one.
