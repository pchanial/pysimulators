...
def test_add1():
    q = Quantity(1.0)
    for other in Quantity(1.0), np.array(1.0), 1:
...
def test_add6():
    assert_raises(UnitError, lambda: Quantity(1, 'kloug') + Quantity(3.0, 'babar'))
...
def test_dtype():
    ...
    assert_is(Quantity(1.0).dtype, np.dtype(float))
...
def test_derived_units4():
    ...
    def func_leftward(a, key):
        ...
        if np.isscalar(b):
            ...
            return
        if key is Ellipsis:
            assert_eq(b.derived_units['r[leftward]'], a.derived_units['r[leftward]'])
        if key in ((1, 2, 3, 4), (_, 1, 2, 3)):
            ...
            assert_eq(
                b.derived_units['r[leftward]'], a.derived_units['r[leftward]'][key[-3:]]
            )
    ...
    def func_rightward(a, key):
        ...
        if np.isscalar(b):
            ...
            return
        if key is Ellipsis:
            assert_eq(b.derived_units['r[rightward]'], a.derived_units['r[rightward]'])
