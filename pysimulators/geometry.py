...
def create_grid_squares(shape, spacing, out=None, filling_factor=1,
                        xreflection=False, yreflection=False, center=(0, 0),
                        angle=0):
    """
    ...
    _flib.geometry.create_grid_squares(
        spacing, filling_factor, xreflection, yreflection, angle, center[0],
        center[1], out.T)
    return out
...
def create_rectangle(size, out=None, center=(0, 0), angle=0, dtype=float):
    ...
    out[...] = _np.asarray([[0.5, 0.5], [-0.5, 0.5], [-0.5, -0.5],
                            [0.5, -0.5]], dtype) * size[..., None, :]
    if angle != 0:
