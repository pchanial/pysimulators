...
class FSRRotation3dMatrix(_FSRotation3dMatrix):
    ...
    def _matvec(self, v, out=None):
        ...
        for i in xrange(self.shape[0] // 3):
            ...
            out[i, 1] += np.sum(b.r22 * v[b.index, 1] - b.r32 * v[b.index, 2])
            out[i, 2] += np.sum(b.r32 * v[b.index, 1] + b.r22 * v[b.index, 2])
        return out_
...
class SparseOperator(pyoperators.core.SparseBase):
    def __init__(self, arg, dtype=None, **keywords):
        if sp.issparse(arg):
            ...
            return
        if not isinstance(
            arg, (FSCMatrix, FSRMatrix, FSCRotation3dMatrix, FSRRotation3dMatrix)
        ):
        ...
        self.set_rule(
            'T', lambda s: SparseOperator(s.matrix._transpose(), dtype=s.dtype)
        )
