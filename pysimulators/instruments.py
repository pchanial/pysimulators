...
class Instrument(object):
    ...
    def get_pointing_matrix(
        ...
        **keywords,
        ...
        shape_input = tuple(
            header['NAXIS' + str(i + 1)] for i in range(header['NAXIS'])
        )[::-1]
        ...
        try:
            pmatrix = PointingMatrix.empty(shape, shape_input, info=info, verbose=False)
        except MemoryError:
            gc.collect()
            pmatrix = PointingMatrix.empty(shape, shape_input, info=info, verbose=False)
        ...
        if method == 'sharp':
            ...
            new_npixels_per_sample, outside = self.instrument2pmatrix_sharp_edges(
                coords, pointing, header, pmatrix, npixels_per_sample
            )
        elif method == 'nearest':
            coords = self.pack(self.get_centers())
            new_npixels_per_sample = 1
            outside = self.instrument2pmatrix_nearest_neighbour(
        ...
        del pmatrix
        return self.get_pointing_matrix(
            pointing,
            header,
    ...
    @staticmethod
    def instrument2pmatrix_sharp_edges(
        coords, pointing, header, pmatrix, npixels_per_sample
    ):
        """
    ...
    @staticmethod
    def instrument2pmatrix_sharp_edges_explicit(
        coords, pointing, header, pmatrix, npixels_per_sample
    ):
        """
        Return the sparse pointing matrix whose values are intersection between
        ...
        (
            ...
            out,
            status,
        ) = flib.wcsutils.instrument2pmatrix_sharp_edges_explicit(
            coords.T, ra, dec, pa, masked, header, pmatrix, npixels_per_sample
        )
        if status != 0:
            raise RuntimeError()
    ...
    @staticmethod
    def instrument2pmatrix_sharp_edges_vect(
        coords, pointing, header, pmatrix, npixels_per_sample
    ):
        """
        Return the sparse pointing matrix whose values are intersection between
        ...
        (
            ...
            out,
            status,
        ) = flib.wcsutils.instrument2pmatrix_sharp_edges_vect(
            coords.T, ra, dec, pa, masked, header, pmatrix, npixels_per_sample
        )
        if status != 0:
            raise RuntimeError()
