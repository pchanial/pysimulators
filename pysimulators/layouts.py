...
from pyoperators.utils import (
    ilast_is_not, isalias, isclassattr, isscalarlike, product, split,
    tointtuple)
...
class Layout(object):
    ...
    def _get_index(self, selection, ordering):
        ...
        if ordering is None:
            ...
            out = np.asarray(np.argsort(selection.ravel())[-npacked:],
                             self._dtype_index)
            return self._normalize_int_selection(out, product(self.shape))
        ...
        if npacked == 0:
            return np.array([], self._dtype_index)
        out = np.asarray(np.argsort(ordering.ravel())[-npacked:],
                         self._dtype_index)
        return self._normalize_int_selection(out, product(self.shape))
    ...
    def __setattr__(self, key, value):
        if key in self._reserved_attributes:
            raise AttributeError(
                'The attribute {0!r} is not writeable.'.format(key))
        if value is not None and not callable(value) and \
           key in self._special_attributes:
    ...
    def __getattribute__(self, key):
        ...
        if key in object.__getattribute__(self, '_special_attributes') and \
           callable(value):
    ...
    def __getitem__(self, selection):
        ...
        if index is Ellipsis and \
             (isinstance(selection, slice) or
              isinstance(selection, np.ndarray) and selection.dtype == int):
            index = selection
    ...
    def __str__(self):
        ...
        attributes = ['index'] + sorted(_ for _ in self._special_attributes
                                        if _ not in ('index', 'removed'))
        if len(attributes) > 1:
            out += '\n    '
        out += ',\n    '.join('{0}={1}'.format(_, getattr(self, _))
                              for _ in attributes) + ')'
        return out
    ...
    def unpack(self, x, out=None, missing_value=None, copy=False):
        ...
        if has_out:
            ...
            if self._index is Ellipsis and not copy and \
        ...
        if product(self.shape) > len(self):
            ...
            out[...] = missing_value
        elif self._index is Ellipsis and x.shape[0] == len(self) and \
             self.comm.size == 1:
...
#        return self._packed[selection[index]]
    ...
    def __setattr__(self, key, value):
        ...
        if key not in p._special_attributes:
            raise AttributeError(
                '{0!r} is not an unpacked attribute.'.format(key))
        if key in p._reserved_attributes:
            raise AttributeError(
                'The attribute {0!r} is not writeable.'.format(key))
        if callable(value):
            raise TypeError(
                'A function cannot be set as an unpacked attribute.')
...
class LayoutSpatialGridCircles(LayoutSpatialGrid):
    ...
    ordering : array-like of int, optional
        ...
        LayoutSpatialGrid.__init__(self, shape, spacing, radius=radius,
                                   **keywords)
