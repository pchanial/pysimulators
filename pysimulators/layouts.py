...
class Layout(object):
    ...
    def setattr_unpacked(self, key, value):
        ...
        if key in self._reserved_attributes:
            raise KeyError(
                'A special layout attribute cannot be {0}.'.format(
    ...
    def setattr_packed(self, key, value):
        ...
        if callable(value):
            ...
            if key == 'index' and value is not None:
                if value.size == len(self) and np.all(value == np.arange(len(self))):
                    value = None
    ...
    def _pack_index(index, removed):
        if isscalarlike(removed):
            ...
            if index is None:
                index = np.arange(removed.size, dtype=np.int32).reshape(removed.shape)
            index[removed] = -1
        index = index.ravel()
...
class LayoutGrid(Layout):
    ...
    def __init__(
        ...
        **keywords,
        ...
        self.angle = angle
        self._unit = unit
        Layout.__init__(self, shape, center=center, column=column, row=row, **keywords)
