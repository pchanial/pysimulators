...
class PackedTable(object):
    ...
    def _get_index(self, selection, ordering):
        ...
        if ordering is None:
            ...
            return self._normalize_int_selection(
                np.asarray(np.where(selection.ravel())[0],
    ...
    def __getattribute__(self, key):
        if key == 'packed':
            warn("Please update your code: the 'packed' attribute is not requi"
                 "red anymore: the attributes are already packed. To access th"
                 "e unpacked ones, use the 'all' attribute.",
                 PySimulatorsWarning)
            return self
        value = object.__getattribute__(self, key)
        if key not in object.__getattribute__(self, '_special_attributes') or \
           not callable(value):
            return value
    ...
    def __str__(self):
        ...
        attributes = ['index'] + sorted(_
            for _ in self._special_attributes if _ not in ('index', 'removed')
            and not isinstance(object.__getattribute__(self, _),
                               (types.FunctionType, types.MethodType,
                                types.NoneType)))
        if len(attributes) > 1:
            out += '\n    '
        out += ',\n    '.join('{0}={1}'.format(_, str(getattr(self, _))[:65])
                              for _ in attributes) + ')'
        return out
