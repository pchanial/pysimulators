...
def all_eq(a, b, rtol=None, atol=0.0):
    return not any_neq(a, b, rtol=rtol, atol=atol)


def any_neq(a, b, rtol=None, atol=0.0):
    """
    ...
    if rtol is None:
        ...
        if precision == 8:
            ...
            raise NotImplementedError(
                'The inputs are not in single or double' ' precision.'
            )
    ...
    if result:
        ...
        print(
            'Argument data difference exceeds tolerance by factor ' + str(factor) + '.'
        )
...
def assert_all_eq(a, b, rtol=None, atol=0.0, msg=None):
...
def hs(arg):
    ...
    if isinstance(arg, np.ndarray):
        ...
        members = inspect.getmembers(
            arg, lambda x: not inspect.ismethod(x) and not inspect.isbuiltin(x)
        )
...
def diff(input, output, axis=0, comm=None):
    ...
    if input.dtype != float:
        raise TypeError(
            'The data type of the input array is not ' + str(float.type) + '.'
        )
    ...
    if axis >= ndim:
        raise ValueError(
            "Invalid axis '" + str(axis) + "'. Expected value is"
            ' less than ' + str(ndim - 1) + '.'
        )

    inplace = (
        input.__array_interface__['data'][0] == output.__array_interface__['data'][0]
    )
...
def diffT(input, output, axis=0, comm=None):
    ...
    with filter_comm(input.shape[0] > 0, comm) as fcomm:
        if fcomm is not None:
            status = tmf.operators_mpi.difft(
                input.ravel(),
                output.ravel(),
                ndim - axis,
                np.asarray(input.T.shape),
...
def diffTdiff(input, output, axis=0, scalar=1.0, comm=None):
    """
    ...
    with filter_comm(input.shape[0] > 0, comm) as fcomm:
        if fcomm is not None:
            status = tmf.operators_mpi.difftdiff(
                input.ravel(),
                output.ravel(),
                ndim - axis,
                np.asarray(input.T.shape),
...
def shift(input, output, n, axis=-1):
    ...
    if input.dtype != float:
        raise TypeError(
            'The data type of the input array is not ' + str(float.name) + '.'
        )
    ...
    if (
        ...
        and n.size > 1
        or n.size != 1
        and n.size not in np.cumproduct(input.shape[0:axis])
    ):
        raise ValueError(
            'The offset size is incompatible with the first dime' 'nsions of the array'
        )
    if rank == 0:
        output[()] = 0
    else:
        tmf.shift(
            input.ravel(), output.ravel(), rank - axis, np.asarray(input.T.shape), n
        )
