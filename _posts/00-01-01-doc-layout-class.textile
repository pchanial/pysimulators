...
>>> gain = [[1.0, 1.0, 1.0],
            [0.9, 1.0, 1.0],
            [0.8, 1.0, 1.0]]
>>> layout = LayoutGridSquares(shape, spacing, removed=removed,
                               gain=gain)
>>> len(layout)
9
>>> len(layout.packed)
...
And conversely, any "packed" array can be unpacked. By default, for removed components, -1 is used for integer arrays and NaN otherwise, but this value can be specified with the @removed_value@ keyword.
...
* the removed mask cannot be updated once the layout is created.
* only modifications to the packed arrays are persistent.
...
These functions have no arguments and as a result, they should use parent scope variables. Note that Python's handling of variable scoping may be unintuitive and is a frequent source of mistakes. It is necessary to check that these parent scope variables are not reassigned to another value after the function is defined, since the new values would be the ones used in the function.

A special attribute can also be a scalar. In this case, both packed and unpacked values are scalars.
